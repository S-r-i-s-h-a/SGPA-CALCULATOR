...
public class cgpa  extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        try{
            ...
            Double gp2=Double.parseDouble(et1.getText().toString());
            Double gp3=Double.parseDouble(et2.getText().toString());
            Double gp4=Double.parseDouble(et3.getText().toString());
            Double gp5=Double.parseDouble(et4.getText().toString());
            Double gp6=Double.parseDouble(et5.getText().toString());
            Double gp8=Double.parseDouble(et7.getText().toString());
            Double gp7=Double.parseDouble(et6.getText().toString());
            Double gp1=Double.parseDouble(et.getText().toString());
           // int gp1= (int) cred(m1);
            //int gp2= (int) cred(m2);
            //int gp3= (int) cred(m3);
            //int gp4= (int) cred(m4);
            ...
            double cg=((gp1*c1) +(gp2*c2)+(gp3*c3)+(gp4*c4)+(gp5*c5)+(gp6*c6)+(gp7*c7)+(gp8*c8))/(c1+c2+c3+c4+c5+c6+c7+c8);
            double cgpa=Math.round(cg*100.00)/100.00;
            double per= (cgpa-0.75)*10.00;
            double perc=Math.round(per*100.00)/100.00;


            TextView myTextView =findViewById(R.id.textView11);

            myTextView.setText("CGPA="+cgpa+" "+"per="+perc);
            String TAG=" errrrrrrr";
            Log.d(TAG, "onClick: "+cgpa+ " "+ perc);}
        catch (Exception e)
        {
